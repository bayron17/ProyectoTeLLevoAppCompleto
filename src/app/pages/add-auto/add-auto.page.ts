...
export class AddAutoPage implements OnInit {
  ...
  async viewAuto(){
    await this.storageAuto.obtenerAuto();
  }
  ...
  async agregaV(){
    ...
    if(this.marca.length == 0|| this.modelo.length == 0|| this.patente .length== 0|| this.color.length == 0|| this.capacidad.length == 0|| this.duenio.length  == 0 || this.anio.length == 0){
      ...
      if (storageauto) { 
        ...
        console.log(this.leru);
        if(this.leru){
          this.helper.showAlert("No puede ingresar dos vehiculo con la misma patente","Vehiculo ya registrado")
        }else{
